...
import {
  calculateNecessaryPaper,
  calculateNecessaryRibbon,
  extractDimension,
} from "./script";
...
test("part1: actual input", () => {
  ...
  file.split(/\r?\n/).forEach((line: string) => {
    ...
    if (!trimmedLine) {
      ...
      necessaryPaperSum += calculateNecessaryPaper(
        extractDimension(trimmedLine),
      );
    }
  });
...
test("part2: actual input", () => {
  ...
  file.split(/\r?\n/).forEach((line: string) => {
    ...
    if (!trimmedLine) {
      ...
      necessaryRibbonSum += calculateNecessaryRibbon(
        extractDimension(trimmedLine),
      );
    }
  });
