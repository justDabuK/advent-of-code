...
test.each([
  ...
  expect(deliverPresentsWithRoboSanta(convertToDirectionList(input))).toBe(
    expected,
  );
});
...
test("part2: actual input", () => {
  ...
  expect(
    deliverPresentsWithRoboSanta(convertToDirectionList(instructionString)),
  ).toBe(2341);
});
