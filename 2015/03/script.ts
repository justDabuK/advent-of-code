...
export function deliverPresents(directionList: Direction[]): number {
    ...
    directionList.forEach((direction: Direction) => {
        ...
        currentPosition = { x: currentPosition.x + delta.x, y: currentPosition.y + delta.y };
        visitedHouses.push(currentPosition);
    });

    const visitedHousesWithoutDuplicates = new Set(visitedHouses.map((position: Position) => `${position.x}-${position.y}`));
    return visitedHousesWithoutDuplicates.size;
}

export function deliverPresentsWithRoboSanta(directionList: Direction[]): number {
    const visitedHouses: Position[] = [];
    ...
    directionList.forEach((direction: Direction) => {
        const delta = directionDeltaMap[direction];
        if(santasTurn){
            santaPosition = { x: santaPosition.x + delta.x, y: santaPosition.y + delta.y };
            visitedHouses.push(santaPosition);
        } else {
            roboSantaPosition = { x: roboSantaPosition.x + delta.x, y: roboSantaPosition.y + delta.y };
            visitedHouses.push(roboSantaPosition);
        }
