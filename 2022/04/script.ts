...
function isContaining(crew: CleanupCrew): boolean {
    if(crew.firstElf.start <= crew.secondElf.start && crew.firstElf.end >= crew.secondElf.end) {
        return true;
    } else if (crew.firstElf.start >= crew.secondElf.start && crew.firstElf.end <= crew.secondElf.end) {
        return true;
    } else {
        return false;
    }
...
function isInsideBoundaries(value: number, lowerBound: number, upperBound: number): boolean {
...
function isOverlap(crew: CleanupCrew): boolean {
    ...
    return isInsideBoundaries(firstElfStart, secondElfStart, secondElfEnd)
