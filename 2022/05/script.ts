...
function applyAllCommands(supplies: Supplies, commandConsumer: (stackList: string[][], command: RearrangeCommand) => void) {
    supplies.commands.forEach((command) => commandConsumer(supplies.stackList, command));
}
...
function applyCommand9001(stackList: string [][], command: RearrangeCommand) {
    ...
    stackList[command.to-1] = stackList[command.to-1].concat(stackList[command.from - 1].slice(sliceIndex));
    stackList[command.from-1] = stackList[command.from - 1].slice(0, sliceIndex);
}
