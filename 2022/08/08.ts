...
function getData(fileName: string): number[][] {
    ...
    file.split(/r?\n/).forEach((line: string) => {
        ...
        if(!trimmedLine) {
            ...
            treeHeightMatrix.push(trimmedLine.split('').map((height) => parseInt(height)));
        }
    })
...
function isVisibleFromTop(treeMatrix: number[][], row: number, col: number): boolean {
...
function isVisibleFromRight(treeMatrix: number[][], row: number, col: number): boolean {
    const currentTreeHeight = treeMatrix[row][col];
...
function isVisibleFromBottom(treeMatrix: number[][], row: number, col: number): boolean {
    const currentTreeHeight = treeMatrix[row][col];
...
function isVisibleFromLeft(treeMatrix: number[][], row: number, col: number): boolean {
    const currentTreeHeight = treeMatrix[row][col];
...
function getVisibleTreeCount(treeMatrix: number[][]): number {
    ...
    for (let row = 0; row < rowLength; row++){
        for(let col = 0; col < columnLength; col++) {
            if(row === 0 || col === 0 || row === rowLength - 1 || col === columnLength - 1) {
                visibleCounter++;
            } else {
...
function getVisibleTreeCountFromTop(treeMatrix: number[][], row: number, col: number): number {
    if(row === 0) {
        return 0;
    }
...
function getVisibleTreeCountFromRight(treeMatrix: number[][], row: number, col: number): number {
    const maxColumn = treeMatrix[row].length;
    if(col === maxColumn - 1) {
        return 0;
    }
...
function getVisibleTreeCountFromBottom(treeMatrix: number[][], row: number, col: number): number {
    const maxRow = treeMatrix.length;
    if(row === maxRow - 1) {
        return 0;
    }
...
function getVisibleTreeCountFromLeft(treeMatrix: number[][], row: number, col: number): number {
    if(col === 0) {
        return 0;
    }
...
function createScenicScoreMatrix(treeMatrix: number[][]): number[][] {
    ...
    for (let row = 0; row < rowLength; row++){
        ...
        for(let col = 0; col < columnLength; col++) {
            ...
            const treesSeenOnRight = getVisibleTreeCountFromRight(treeMatrix, row, col);
            const treesSeenOnBottom = getVisibleTreeCountFromBottom(treeMatrix, row, col);
            const treesSeenOnLeft = getVisibleTreeCountFromLeft(treeMatrix, row, col);
            const scenicScore = treesSeenOnTop * treesSeenOnRight * treesSeenOnBottom * treesSeenOnLeft;
...
function part2() {
    ...
    const highestScorePerRowList = scoreMatrix.map((scoreRow) => Math.max(...scoreRow));
