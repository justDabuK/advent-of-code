...
function getData(fileName: string): BallGame[] {
    ...
    file.split(/\r?\n/).forEach((line: string) => {
        ...
        if (!trimmedLine) {
            ...
            const gameId = parseInt(gameIdRegExMathArray ? gameIdRegExMathArray[0] : '0');
            ...
            ballDrawList.forEach((ballDraw: string) => {
                ...
                ballTupleList.forEach((ballTuple: string) => {
                    ...
                    const numberOfBalls = parseInt(numberOfBallsRegExMatchArray ? numberOfBallsRegExMatchArray[0] : '0');
...
function part1() {
    ...
    const idSum = filteredGameList.reduce((acc: number, game: BallGame) => acc + game.id, 0);
    console.log(idSum);
}
...
function part2() {
    const gameList = getData('2023/02/input.txt');
    const powerOfSets = gameList.map(toPowerOfSet).reduce((acc: number, value: number) => acc + value, 0);
