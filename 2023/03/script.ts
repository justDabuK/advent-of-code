...
function part1() {
  ...
  console.log(
    `symbolPositionListViaRegex.length: ${symbolPositionListViaRegex.length} vs. symbolPositionList.length: ${symbolPositionList.length}`,
  );
  ...
  const filteredNumberList = filterNumbers(
    numberListViaRegex,
    symbolPositionList,
  );
  //console.table(filteredNumberList);
  // sum up adjacent numbers
  const sum = filteredNumberList.reduce(
...
function part2() {
  ...
  const gearSum = filteredGears
    .map(
      (gear) =>
        gear.adjacentNumbers?.reduce(
