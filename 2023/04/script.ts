...
function getData(fileName: string): ScratchCard[] {
  ...
  file.split(/\r?\n/).forEach((line: string) => {
    ...
    if (!trimmedLine) {
      ...
      const winningNumberList = numberListSplit[0]
        .match(/\d+/g)!
        .map((numberAsString) => parseInt(numberAsString));
      const myNumberList = numberListSplit[1]
        .match(/\d+/g)!
        .map((numberAsString) => parseInt(numberAsString));
...
function part2() {
  ...
  const totalNumberOfCards = cardList.reduce(
    (sum, card) => sum + card.numberOfInstances,
    0,
  );
