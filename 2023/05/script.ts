...
function loadDataPart1(fileName: string): void {
    ...
    file.split(/\r?\n/).forEach((line: string) => {
        ...
        if (!trimmedLine) {
            ...
            switch (currentMode) {
                case DataMode.seeds:
                    seedList = trimmedLine.split(':')[1].match(/\d+/g)!.map((seed) => BigInt(seed));
                    break;
...
function toLoacationList(seedList: bigint[]): bigint[] {
    return seedList
        ...
        .map((temperature) => mapValueToRange(temperature, temperatureToHumidityMap))
...
function part1() {
    ...
    const minLocation = locationList.reduce((min, location) => location > min ? min : location, locationList[0]);
            ...
            switch (currentMode) {
                case DataMode.seeds:
                    seedPairList = trimmedLine
                        ...
                        .match(/\d+ \d+/g)!
                        .map((pair) => {
                        const numberList = pair.split(' ').map((number) => BigInt(number));
                        return {
                            start: numberList[0],
