    ...
    file.split(/\r?\n/).forEach((line: string) => {
        ...
        if (!trimmedLine) {
            ...
            if (trimmedLine.includes("Time")) {
                raceTimeList = trimmedLine.split(':')[1].match(/\d+/g)!.map((numberAsString) => parseInt(numberAsString));
            } else if (trimmedLine.includes("Distance")) {
                recordDistanceList = trimmedLine.split(':')[1].match(/\d+/g)!.map((numberAsString) => parseInt(numberAsString));
            } else {
...
function part1() {
    ...
    const multipliedRecordBeatNumbers = raceRecordList.map((raceRecord) => getLongerPossibleOutcomes(raceRecord).length).reduce((multiplier, value) => multiplier * value, 1);
