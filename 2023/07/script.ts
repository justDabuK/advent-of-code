...
function getData(fileName: string): Hand[] {
    ...
    file.split(/\r?\n/).forEach((line: string) => {
        ...
        if (!trimmedLine) {
            ...
            const cardList = splitLine[0].split('').map((cardAsString) => toCard(cardAsString));
...
function part1() {
    ...
    const combinedRank = sortedHandList.map((hand, index) => hand.bid * (index + 1)).reduce((a, b) => a + b, 0);
