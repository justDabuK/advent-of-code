...
function part2() {
  const { instructionList, nodeMap } = getData("./2023/08/input.txt");
  let currentNodeList = Object.keys(nodeMap).filter((nodeName) =>
    nodeName.endsWith("A"),
  );
  let numberOfSteps = 0;
  do {
    for (const instruction of instructionList) {
      if (instruction === "L") {
        currentNodeList = currentNodeList.map(
          (nodeName) => nodeMap[nodeName].leftChild,
        );
      } else {
        currentNodeList = currentNodeList.map(
          (nodeName) => nodeMap[nodeName].rightChild,
        );
      }

      numberOfSteps++;

      const allNodesAreInTheGoal = currentNodeList.every((nodeName) =>
        nodeName.endsWith("Z"),
      );
