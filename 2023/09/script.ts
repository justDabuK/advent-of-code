...
function getData(fileName: string): number[][] {
  ...
  file.split(/\r?\n/).forEach((line: string) => {
    ...
    if (!trimmedLine) {
      ...
      listOfReadingList.push(
        splitLine.map((numberAsString) => parseInt(numberAsString)),
      );
    }
  });
...
function predictNextReading(readingList: number[]): number {
  ...
  do {
    ...
    !predictionLayers[predictionLayers.length - 1].every(
      (number) => number === 0,
    )
  );
  predictionLayers.reverse().forEach((layer, index) => {
    if (index === 0) {
      ...
      layer.push(
        predictionLayers[index - 1][predictionLayers[index - 1].length - 1] +
          layer[layer.length - 1],
      );
    }
  });
  return predictionLayers[predictionLayers.length - 1][
    predictionLayers[predictionLayers.length - 1].length - 1
  ];
}
...
function part1() {
  ...
  console.log(
    readingList
      .map((reading) => predictNextReading(reading))
...
function part2() {
  ...
  console.log(
    readingList
      .map((reading) => predictPreviousReading(reading))
      .reduce((sum, current) => sum + current, 0),
  );
}
