...
function part1() {
  ...
  for (let i = 0; i < dataTable[0].length; i++) {
    ...
    sum += result;
    console.log(
      `|${dataTable[0][i]} - ${dataTable[1][i]}| = ${result} => ${sum}`,
    );
  }
  console.groupEnd();
}
...
function part2() {
  ...
  const dataTable = getData("./input.txt");
  const occurrenceCounter = dataTable[1].reduce(
  ...
  for (let i = 0; i < dataTable[0].length; i++) {
    ...
    const result =
      occurrenceCounter[numberOfInterest] !== undefined
        ? numberOfInterest * occurrenceCounter[numberOfInterest]
        : 0;
    sum += result;
  }
  console.log("Sum:", sum);
