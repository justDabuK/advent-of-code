...
function part1() {
  ...
  const numberOfSafeReports = reportResultList.reduce(
...
function part2() {
  console.group("part2");
  const dataTable = getData("./input.txt");
  const reportResultList = dataTable.map((report) => [
    ...report,
    isReportSafe(report),
    isReportSafeTryouts(report),
  ]);
  console.table(reportResultList);
  const numberOfSafeReports = reportResultList.reduce(
    (acc, curr) => (curr[curr.length - 1] ? acc++ : () => {}, acc),
    0,
  );
  console.log("number of safe reports", numberOfSafeReports);
  console.groupEnd();
