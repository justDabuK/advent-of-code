    ...
    file.split(/\r?\n/).forEach((line: string) => {
        ...
        if (!trimmedLine) {
            ...
            while (regExpMatchArray !== null) {
                numberListList.push([parseInt(regExpMatchArray[1]), parseInt(regExpMatchArray[2])]);
...
function part1() {
    ...
    console.table(data);
    const multipliedData = data.map((numbers) => numbers.reduce((acc, number) => acc * number, 1));
