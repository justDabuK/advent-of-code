      ...
      if (character === "X") {
        ...
        Object.keys(directionMap).forEach((direction) => {
          ...
          if (
            ...
            nextRow < 0 ||
            nextRow >= characterMatrix.length
          ) {
            if (debug) console.log("out of bounds");
            return;
          }
          if (
            characterMatrix[nextRow] &&
            characterMatrix[nextRow][nextColumn] === "M"
          ) {
            ...
            if (
              characterMatrix[nextRow] &&
              characterMatrix[nextRow][nextColumn] === "A"
            ) {
              ...
              if (
                characterMatrix[nextRow] &&
                characterMatrix[nextRow][nextColumn] === "S"
              ) {
                if (debug)
                  console.log(`FOUND S at ${nextRow} x ${nextColumn}, xmas++`);
                xmasCounter++;
...
function findCrossMas(characterMatrix: string[][], debug = false): number {
  ...
  const checkAtPosition = (
    ...
    if (
      column < 0 ||
      column >= characterMatrix[0].length ||
      row < 0 ||
      row >= characterMatrix.length
    ) {
      if (debug) console.log("out of bounds");
      return false;
    }
    ...
    line.forEach((character: string, column) => {
      ...
      if (character === "A") {
        ...
        Object.keys(crossOrientationMap).forEach((orientation) => {
          ...
          for (
            let locationIndex = 0;
            locationIndex < characterLocations.length;
            locationIndex++
          ) {
            if (
              !checkAtPosition(
              ...
              if (debug)
                console.log(
                  `FAIL for ${characterLocations[locationIndex].character} at ${row + characterLocations[locationIndex].rowDelta} x ${column + characterLocations[locationIndex].columnDelta}`,
                );
              return;
            }
            if (debug)
              console.log(
                `FOUND ${characterLocations[locationIndex].character} at ${row + characterLocations[locationIndex].rowDelta} x ${column + characterLocations[locationIndex].columnDelta}`,
              );
          }
          if (debug)
            console.log(
              `FOUND CROSS at ${row} x ${column} with orientation ${orientation}, crossMass++`,
            );
          crossMassCounter++;
        });
      } else {
