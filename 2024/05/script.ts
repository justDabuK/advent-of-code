...
function isUpdateInRightOrder(
  ...
  for (
    ...
    currentUpdateIndex < pageUpdate.length - 1;
    currentUpdateIndex++
  ) {
    for (
      let comparingUpdateIndex = 0;
      comparingUpdateIndex < pageUpdate.length;
      comparingUpdateIndex++
    ) {
      ...
      if (rule) {
        if (currentUpdateIndex < comparingUpdateIndex) {
          if (
            !(rule.before === currentUpdate && rule.after === comparingUpdate)
          ) {
            if (debug)
              console.log(
                `FAIL ${currentUpdate}|${comparingUpdate} against ${rule.before}|${rule.after}`,
              );
            return false;
          }
        } else {
          if (
            !(rule.before === comparingUpdate && rule.after === currentUpdate)
          ) {
            if (debug)
              console.log(
                `FAIL ${currentUpdate}|${comparingUpdate} against ${rule.before}|${rule.after}`,
        ...
        if (debug)
          console.log(
            `SUCCESS ${currentUpdate}|${comparingUpdate} against ${rule.before}|${rule.after}`,
          );
      } else {
        if (debug)
          console.log(
            `No rule found for ${currentUpdate} and ${comparingUpdate}`,
          );
      }
    }
  }
...
function part1() {
  ...
  const sumOfMiddleSuccesses = printerInstructions.pagesUpdateList
    .filter((pageUpdate) =>
      isUpdateInRightOrder(printerInstructions.ruleIndex, pageUpdate),
    )
    .map((pageUpdate) => pageUpdate[Math.floor(pageUpdate.length / 2)])
    .reduce((acc, current) => acc + current, 0);
  console.log(
    "Sum of middle numbers of successful updates",
    sumOfMiddleSuccesses,
  );
}
...
const fixUpdateOrder = (
  ...
  for (
    let currentUpdateIndex = 0;
    currentUpdateIndex < fixedPageUpdate.length - 1;
    currentUpdateIndex++
  ) {
    for (
      let comparingUpdateIndex = 0;
      comparingUpdateIndex < fixedPageUpdate.length;
      comparingUpdateIndex++
    ) {
      if (currentUpdateIndex === comparingUpdateIndex) {
        continue;
      ...
      if (rule) {
        if (currentUpdateIndex < comparingUpdateIndex) {
          if (
            ...
            fixedPageUpdate[currentUpdateIndex] =
              fixedPageUpdate[comparingUpdateIndex];
...
function part2() {
  ...
  const sumOfMiddleSuccesses = printerInstructions.pagesUpdateList
    ...
    .map((pageUpdate) =>
      fixUpdateOrder(printerInstructions.ruleIndex, pageUpdate),
    )
    .map((pageUpdate) => pageUpdate[Math.floor(pageUpdate.length / 2)])
    .reduce((acc, current) => acc + current, 0);
