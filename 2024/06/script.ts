...
function findLoopPositions(originalMap: string[][], originalGuardPath: GuardPath)  {
    ...
    const isDuplicatedPosition = (guardPosition: [number, number], direction: Direction, guardPath: GuardPath): boolean => {
        return guardPath.some((entry) => entry.position[0] === guardPosition[0] && entry.position[1] === guardPosition[1] && entry.direction === direction)
    }

    const loopWallPositions:[number, number][] = []

    for(let originalGuardPathIndex = 1; originalGuardPathIndex < originalGuardPath.length; originalGuardPathIndex++) {
        if(originalGuardPathIndex % 500 === 0) console.log(`loop counter ${loopWallPositions.length} (${originalGuardPathIndex}/${originalGuardPath.length})`)
        const extraWallPosition = originalGuardPath[originalGuardPathIndex].position
        const adjustedMap = JSON.parse(JSON.stringify(originalMap))
        adjustedMap[extraWallPosition[0]][extraWallPosition[1]] = '#'
        const currentGuardPath: { position: [number, number], direction: Direction }[] = []

        let guardPosition: [number, number] = [...initialGuardPosition]
        let guardDirection: Direction = 'up'

        while (isGuardInBounds(guardPosition) && !isDuplicatedPosition(guardPosition, guardDirection, currentGuardPath)) {
            const [currentRow, currentColumn] = guardPosition;
            ...
            const nextRow = currentRow + directionMap[guardDirection][0]
            const nextColumn = currentColumn + directionMap[guardDirection][1]
            if(isGuardInBounds([nextRow, nextColumn]) && adjustedMap[nextRow][nextColumn] === '#') {
                guardDirection = getDirectionAfterTurn(guardDirection)
            } else {
                guardPosition = [nextRow, nextColumn]
            }
        ...
        if(!isGuardInBounds(guardPosition)) {
            // nothing to do
        } else if(isDuplicatedPosition(guardPosition, guardDirection, currentGuardPath)) {
            loopWallPositions.push(extraWallPosition)
        } else {
            console.log(`EXCEPTION: guard position ${guardPosition} (bounds: ${upperRowBound},${upperColumnBound}), direction: ${guardDirection} extra wall position ${extraWallPosition} guard direction ${guardDirection}`)
...
function part2() {
    ...
    const positionSet = new Set(loopWallPositions.map(([row, column]) => `${row}-${column}`));
