...
export const getOperatorList = (
  ...
  entryIndex: number,
  numberOfAllowedMethods = 2,
) => {
  const entryInBinaryString = entryIndex.toString(numberOfAllowedMethods);
  const reversedEntryInBinaryCharacterList = entryInBinaryString
    .split("")
    .reverse();
  ...
  for (
    ...
    const allowedMethodIndex = parseInt(
      reversedEntryInBinaryCharacterList[operatorIndex] || "0",
    );
...
export const createOperatorListList = (
  numberOfOperators: number,
  numberOfAllowedMethods = 2,
) => {
  ...
  for (
    let entryIndex = 0;
    entryIndex < Math.pow(numberOfAllowedMethods, numberOfOperators);
    entryIndex++
  ) {
    operatorListList.push(
      getOperatorList(numberOfOperators, entryIndex, numberOfAllowedMethods),
    );
  }
