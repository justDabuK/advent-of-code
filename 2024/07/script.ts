...
function isSolvable(
  equation: Equation,
  operatorListList: ((a: number, b: number) => number)[][],
  debug = false,
): boolean {
  ...
  for (
    let possibleEquationIndex = 0;
    possibleEquationIndex < operatorListList.length;
    possibleEquationIndex++
  ) {
    let equationResult = equation.operands[0];
    for (
      let operatorIndex = 0;
      operatorIndex < equation.operands.length - 1;
      operatorIndex++
    ) {
      try {
        equationResult = operatorListList[possibleEquationIndex][operatorIndex](
          equationResult,
          equation.operands[operatorIndex + 1],
        );
        if (debug)
          console.log(
            `intermediate Result at operatorIndex ${operatorIndex}`,
            equationResult,
          );
      } catch (e) {
        console.log(
          `exception when equation index: ${possibleEquationIndex} while number of possible equations: ${operatorListList.length}`,
          e,
        );
        break;
      }
...
function part1() {
  ...
  const maxOperators = Math.max(
    ...equationList.map((equation) => equation.operands.length),
  );
  const operatorListList = createOperatorListList(maxOperators);
  console.log(
    `operatorListList (with ${maxOperators} max operators) is finished, gonna see if equations are solvable`,
  );
  const sumOfSolvableEquationResults = equationList
    .filter((equation) => {
