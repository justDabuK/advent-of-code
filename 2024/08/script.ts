...
function registerAntenna(antennaMap: string[][]) {
    const antennaRegister: Map<string, Position[]> = new Map<string, Position[]>();
    for(let rowIndex = 0; rowIndex < antennaMap.length; rowIndex++) {
        for(let columnIndex = 0; columnIndex < antennaMap[rowIndex].length; columnIndex++) {
            ...
            if (potentialAntenna !== '.') {
                ...
                antennaRegister.get(potentialAntenna).push({row: rowIndex, column: columnIndex});
            }
        }
    }
...
function getAntinodes(antennaA: Position, antennaB: Position): Position[] {
    ...
    return [{row: antennaA.row + deltaRow, column: antennaA.column + deltaColumn}, {row: antennaB.row - deltaRow, column: antennaB.column - deltaColumn}];
}
      ...
      const antennaPositions = antennaRegister.get(antennaType);
        for(let i = 0; i < antennaPositions.length; i++) {
            for(let j = i + 1; j < antennaPositions.length; j++) {
                antinodePositions.push(...getAntinodes(antennaPositions[i], antennaPositions[j]));
            }
        }
    }
...
function part1() {
    ...
    antinodePositions = antinodePositions.filter((antinode) => antinode.row >= 0 && antinode.row < antennaMap.length && antinode.column >= 0 && antinode.column < antennaMap[0].length);
    ...
    const antinodeSet = new Set(antinodePositions.map((antinode) => `${antinode.row},${antinode.column}`));
    console.log(antinodeSet.size);
}

const isPositionInBounds = (position: Position, bounds: {minRow: number, maxRow: number, minColumn: number, maxColumn: number}) => {
    return position.row >= bounds.minRow && position.row <= bounds.maxRow && position.column >= bounds.minColumn && position.column <= bounds.maxColumn;
}

function getAntinodesEnhanced(antennaA: Position, antennaB: Position, bounds: {minRow: number, maxRow: number, minColumn: number, maxColumn: number}): Position[] {
    const deltaRow = antennaA.row - antennaB.row;
    const deltaColumn = antennaA.column - antennaB.column;
    ...
    potentialAntinode = {row: antennaA.row - deltaRow, column: antennaA.column - deltaColumn};
    while (isPositionInBounds(potentialAntinode, bounds) && potentialAntinode.column <= bounds.maxColumn) {
        antinodeList.push({ ...potentialAntinode });
        potentialAntinode.row -= deltaRow;
        potentialAntinode.column -= deltaColumn;
    }
...
function getEnhancedAntinodePositionList(antennaRegister: Map<string, Position[]>, bounds: {minRow: number, maxRow: number, minColumn: number, maxColumn: number}) {
    const antinodePositions: Position[] = [];
    for (let [antennaType, antennaPositions] of antennaRegister.entries()) {
      const antennaPositions = antennaRegister.get(antennaType);
        for(let i = 0; i < antennaPositions.length; i++) {
            for(let j = i + 1; j < antennaPositions.length; j++) {
                antinodePositions.push(...getAntinodesEnhanced(antennaPositions[i], antennaPositions[j], bounds));
            }
        }
    }
    return antinodePositions;
...
function part2() {
    ...
    const antennaRegister = registerAntenna(antennaMap);
    if(debug) console.log(antennaRegister);
    let antinodePositions = getEnhancedAntinodePositionList(
        antennaRegister,
        {
            minRow: 0,
