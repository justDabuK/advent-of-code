...
function fragmentDiskLayout(diskLayout: (number | ".")[]): (number | ".")[] {
  const optimizedDiskLayout: (number | ".")[] = JSON.parse(
    JSON.stringify(diskLayout),
  );
...
function part1() {
  ...
  const checksum = optimizedDiskLayout.reduce(
    (acc: number, value: number | ".", currentIndex) =>
      acc + (value === "." ? 0 : value * currentIndex),
    0,
  );
...
function optimizeDiskLayout(diskLayout: (number | ".")[]): (number | ".")[] {
  ...
  let fileLength = 0;
  for (
    let diskIndex = optimizedDiskLayout.length - 1;
    diskIndex >= 0;
    diskIndex--
  ) {
    if (optimizedDiskLayout[diskIndex] !== ".") {
